...
export default function MusicVisualizer({ onBack }: MusicVisualizerProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (customSceneObjects) customSceneObjects.update(bands, time, refs.renderer)
      ...
      <div className={`absolute inset-0 pointer-events-none transition-opacity duration-500 ${showUI ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            {hasSettingsPanel && (
              <button onClick={handleSettingsClick}
                className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 rounded-full border text-xs sm:text-sm transition-all ${isSettingsPanelOpen ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-white/5 hover:bg-white/10 border-white/10 text-white/60 hover:text-white'}`}>
            ...
            <div className="relative">
              ...
              {showPresets && (
                <div className="absolute top-full right-0 mt-2 py-2 bg-black/90 backdrop-blur-md rounded-lg border border-white/10 min-w-[120px] sm:min-w-[140px] z-50">
                  {presetNames.map((id) => (
                    <button key={id} onClick={() => handlePresetChange(id)} className={`w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm transition-all ${currentPresetId === id ? 'text-white bg-white/10' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
        ...
        {isMobile ? (
          <div className="absolute bottom-0 left-0 right-0 pointer-events-auto z-30">
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowModeList(!showModeList) }} className="w-full px-4 py-3 bg-black/80 border-t border-white/10 flex items-center justify-between">
              ...
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-white/40 transition-transform ${showModeList ? 'rotate-180' : ''}`}><path d="M18 15l-6-6-6 6"/></svg>
            </button>
            {showModeList && (
              <div className="bg-black/95 border-t border-white/10 max-h-[50vh] overflow-y-auto">
                {visualizations.map((mode, i) => (
                  <button key={mode.id} onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleModeChange(mode) }} onTouchEnd={(e) => { e.preventDefault(); handleModeChange(mode) }} className={`w-full flex items-center gap-3 px-4 py-4 text-sm transition-all select-none ${currentMode.id === mode.id ? 'bg-emerald-500/20 text-white' : 'text-white/60 active:bg-white/10'}`}>
          ...
          <div className="absolute bottom-4 left-4 pointer-events-auto">
            <div className="flex flex-col gap-1">
              {visualizations.map((mode, i) => (
                <button key={mode.id} onClick={() => handleModeChange(mode)} className={`flex items-center gap-3 px-4 py-2 rounded-lg text-sm transition-all ${currentMode.id === mode.id ? 'bg-gradient-to-r from-emerald-600/40 to-teal-600/40 text-white border border-emerald-500/30' : 'text-white/40 hover:text-white/80 hover:bg-white/5'}`}>
        ...
        {isMobile && !showModeList && (
          <div className="absolute bottom-16 left-1/2 -translate-x-1/2 text-white/20 text-xs pointer-events-none">Tap visualization to hide UI</div>
        )}
