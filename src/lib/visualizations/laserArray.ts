...
function initFixtures() {
  ...
  for (let f = 0; f < MAX_FIXTURES; f++) {
    const fixture: LaserFixture = {
      x: 0,
      y: 50,  // Much higher to reach top of screen
      z: -10,
    ...
    for (let b = 0; b < BEAMS_PER_FIXTURE; b++) {
      ...
      fixture.beams.push({
        angle: spreadAngle,
        pitch: Math.PI * 0.25 + Math.random() * 0.1,  // Wider pitch for screen coverage
        targetAngle: spreadAngle,
        targetPitch: Math.PI * 0.25,
        length: 70,  // Much longer beams
        targetLength: 70,
...
export const laserArray: VisualizationMode = {
  id: 'laser_array',
  name: 'Laser Array',
  description: 'Concert lasers filling the entire screen with frequency-mapped beams',
      ...
      update: (bands: AudioBands, time: number) => {
        ...
        for (let f = 0; f < fixtures.length; f++) {
          ...
          for (let b = 0; b < fixture.beams.length; b++) {
            ...
            switch (beam.frequencyBand) {
              case 'bass': bandSmooth = bands.bassSmooth; break
              case 'mid': bandSmooth = bands.midSmooth; break
              case 'high': bandSmooth = bands.highSmooth; break
              case 'treble': bandSmooth = bands.trebleSmooth; break
            }
            ...
            // Wider pitch range to cover full screen height
            beam.targetPitch = Math.PI * 0.2 + 
              Math.sin(time * 0.5 + f * 0.4) * 0.15 +
              bandSmooth * 0.2 +
              bands.beatIntensity * 0.1
            
            beam.targetLength = 60 + bandSmooth * 40 + bands.beatIntensity * 25
