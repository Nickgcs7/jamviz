...
function initBarStates() {
  ...
  for (let i = 0; i < NUM_BARS; i++) {
    barStates.push({ height: BASE_HEIGHT, targetHeight: BASE_HEIGHT, peakHeight: BASE_HEIGHT, peakHoldTimer: 0, velocity: 0 })
  }
}
...
export const spectrumAnalyzer: VisualizationMode = {
  ...
  initParticles(positions: Float32Array, colors: Float32Array, count: number) {
    ...
    for (let i = 0; i < count; i++) {
      if (i < BAR_PARTICLES) {
        ...
        const t = (barIndex % NUM_BARS) / NUM_BARS
        const [r, g, b] = sampleGradient(currentGradient, t)
        colors[i * 3] = r * REFLECTION_OPACITY; colors[i * 3 + 1] = g * REFLECTION_OPACITY; colors[i * 3 + 2] = b * REFLECTION_OPACITY
        ...
        colors[i * 3] = r * 0.5; colors[i * 3 + 1] = g * 0.5; colors[i * 3 + 2] = b * 0.5
      } else {
        positions[i * 3] = 0; positions[i * 3 + 1] = -200; positions[i * 3 + 2] = 0
      }
    }
  ...
  createSceneObjects(scene: THREE.Scene): SceneObjects {
    barMeshes.forEach(m => scene.remove(m)); barGeometries.forEach(g => g.dispose()); barMaterials.forEach(m => m.dispose())
    peakMeshes.forEach(m => scene.remove(m)); peakGeometries.forEach(g => g.dispose()); peakMaterials.forEach(m => m.dispose())
    reflectionMeshes.forEach(m => scene.remove(m)); reflectionMaterials.forEach(m => m.dispose())
    if (basePlaneMesh) { scene.remove(basePlaneMesh); basePlaneMesh.geometry.dispose() }
    if (basePlaneMaterial) basePlaneMaterial.dispose()

    barMeshes = []; barGeometries = []; barMaterials = []; peakMeshes = []; peakGeometries = []; peakMaterials = []; reflectionMeshes = []; reflectionMaterials = []
    ...
    for (let i = 0; i < NUM_BARS; i++) {
      ...
      const barMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(r, g, b), transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending })
      const barMesh = new THREE.Mesh(barGeom, barMat)
      barMesh.position.set(startX + i * (BAR_WIDTH + BAR_GAP), BASE_HEIGHT / 2, 0)
      scene.add(barMesh)
      barMeshes.push(barMesh); barGeometries.push(barGeom); barMaterials.push(barMat)

      const peakGeom = new THREE.BoxGeometry(PEAK_WIDTH, PEAK_HEIGHT, BAR_WIDTH * 0.3)
      const peakMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(1, 1, 1), transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending })
      const peakMesh = new THREE.Mesh(peakGeom, peakMat)
      peakMesh.position.set(startX + i * (BAR_WIDTH + BAR_GAP), BASE_HEIGHT, 0)
      scene.add(peakMesh)
      peakMeshes.push(peakMesh); peakGeometries.push(peakGeom); peakMaterials.push(peakMat)

      const refMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(r * 0.5, g * 0.5, b * 0.5), transparent: true, opacity: REFLECTION_OPACITY, blending: THREE.AdditiveBlending })
      ...
      scene.add(refMesh)
      reflectionMeshes.push(refMesh); reflectionMaterials.push(refMat)
    }

    const planeGeom = new THREE.PlaneGeometry(totalWidth * 1.5, 5)
    basePlaneMaterial = new THREE.MeshBasicMaterial({ color: 0x111122, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending })
    ...
    return {
      ...
      update: (bands: AudioBands, time: number) => {
        ...
        for (let i = 0; i < NUM_BARS; i++) {
          ...
          // Use smoothingFactor for spring physics
          const spring = smoothingFactor * 0.6
          const damping = 0.7
          ...
          if (state.height >= state.peakHeight) { state.peakHeight = state.height; state.peakHoldTimer = PEAK_HOLD_TIME }
          else if (state.peakHoldTimer > 0) state.peakHoldTimer -= dt
          else { state.peakHeight -= PEAK_DECAY_RATE * (MAX_BAR_HEIGHT - BASE_HEIGHT); state.peakHeight = Math.max(state.height, state.peakHeight) }

          const barMesh = barMeshes[i], barMat = barMaterials[i]
          if (barMesh && barMat) {
          ...
          const peakMesh = peakMeshes[i], peakMat = peakMaterials[i]
          if (peakMesh && peakMat) {
            ...
            peakMat.color.setRGB(peakActive ? 1 : pr, peakActive ? 1 : pg, peakActive ? 1 : pb)
            peakMat.opacity = peakActive ? 0.95 : 0.6
          }

          const refMesh = reflectionMeshes[i], refMat = reflectionMaterials[i]
          if (refMesh && refMat) {
  ...
  animate(positions: Float32Array, originalPositions: Float32Array, sizes: Float32Array, colors: Float32Array, count: number, bands: AudioBands, time: number) {
    ...
    for (let i = 0; i < BAR_PARTICLES; i++) {
      ...
      if (barIndex < NUM_BARS) {
        ...
        } else { positions[i * 3 + 1] = -200; sizes[i] = 0 }
      }
    }
    ...
    for (let i = 0; i < REFLECTION_PARTICLES; i++) {
      ...
      if (barIndex < NUM_BARS && particleIndex < count) {
        ...
        colors[particleIndex * 3] = r * fade * REFLECTION_OPACITY; colors[particleIndex * 3 + 1] = g * fade * REFLECTION_OPACITY; colors[particleIndex * 3 + 2] = b * fade * REFLECTION_OPACITY
    ...
    for (let i = 0; i < AMBIENT_PARTICLES; i++) {
      ...
      const drift = Math.sin(time + i * 0.3) * 2, rise = Math.sin(time * 0.5 + i * 0.1) * 1.5
      positions[particleIndex * 3] = ox + drift; positions[particleIndex * 3 + 1] = oy + rise; positions[particleIndex * 3 + 2] = oz + Math.sin(time * 0.3 + i * 0.2)
...
export function setSpectrumGradient(gradient: typeof currentGradient) { currentGradient = gradient }
export function setSpectrumSmoothing(factor: number) { smoothingFactor = Math.max(0.05, Math.min(0.5, factor)) }
